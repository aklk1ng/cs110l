...
fn main() {
    ...
    match ps_utils::get_target(target) {
        Ok(target_process) => match target_process {
            Some(process) => {
                process.print();
                let child_processes =
                    ps_utils::get_child_processes(process.pid).expect("Not found child process");
            ...
            None => {
                println!(
                    "Process \"{}\" did not match any running PIDs or executables",
                    target
                );
                std::process::exit(1);
            }
